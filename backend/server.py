...
async def seed_data(force: bool = False):
    """Seed the database with sample data. Skips if data already exists unless force=true."""
    try:
        # Skip seeding if data already exists — preserves user edits across reloads
        if not force:
            existing = await db.airports.count_documents({})
            if existing > 0:
                return {
                    "success": True,
                    ...
                    "data": {
                        "airports": existing,
                        "aircraft": await db.aircraft.count_documents({}),
                        "flights": await db.flights.count_documents({}),
                        "routes": await db.route_analytics.count_documents({})
                    }
                }
...
@api_router.get("/analytics/delay-risk")
async def get_delay_risk():
    """
    Compute a delay-risk score (0–100) for every scheduled/boarding flight
    departing in the next 24 hours.

    Algorithm:
      base_score   = historical avg delay on this route  (0–50 pts)
      load_score   = avg load factor on this route       (0–25 pts)
      time_mult    = 1.35 if departure falls in a peak hour, else 1.0
      risk_score   = min(100, (base_score + load_score) × time_mult)
    """
    now          = datetime.now(timezone.utc)
    window_end   = now + timedelta(hours=24)

    upcoming = await db.flights.find(
        {
            "status": {"$in": ["scheduled", "boarding"]},
            "departure_time": {
                "$gte": now.isoformat()[:19],
                "$lte": window_end.isoformat()[:19],
            },
        },
        {"_id": 0},
    ).sort("departure_time", 1).to_list(200)

    # Build a route → analytics lookup for O(1) access
    route_docs = await db.route_analytics.find({}, {"_id": 0}).to_list(1000)
    route_map  = {r["route"]: r for r in route_docs}

    # Peak hours: morning rush 06-09, evening rush 15-19 (local airport time approximation)
    PEAK_HOURS = set(range(6, 10)) | set(range(15, 20))

    results = []
    for f in upcoming:
        route_key  = f"{f['origin_code']}-{f['destination_code']}"
        r_data     = route_map.get(route_key, {})

        hist_delay = r_data.get("avg_delay_minutes", 18.0)
        avg_load   = r_data.get("avg_load_factor",   70.0)

        dep_str    = f["departure_time"]
        dep_dt     = datetime.fromisoformat(dep_str) if isinstance(dep_str, str) else dep_str
        hour       = dep_dt.hour
        is_peak    = hour in PEAK_HOURS

        # Score components
        base_score = min(50.0, (hist_delay / 90.0) * 50.0)
        load_score = (avg_load / 100.0) * 25.0
        mult       = 1.35 if is_peak else 1.0

        risk_score = round(min(100.0, (base_score + load_score) * mult), 1)
        ...
        results.append({
            ...
            "factors": {
                "historical_avg_delay_min": round(hist_delay, 1),
                "peak_hour":                is_peak,
                "avg_load_factor_pct":      round(avg_load, 1),
            },
        })
...
async def update_flight(flight_id: str, flight_data: FlightUpdate):
    """Update an existing flight"""
    existing = await db.flights.find_one({"id": flight_id}, {"_id": 0})
    if not existing:
        raise HTTPException(status_code=404, detail="Flight not found")

    update_fields = {k: v for k, v in flight_data.model_dump().items() if v is not None}
    if "departure_time" in update_fields:
        update_fields["departure_time"] = update_fields["departure_time"].isoformat()
    if "arrival_time" in update_fields:
        update_fields["arrival_time"] = update_fields["arrival_time"].isoformat()

    await db.flights.update_one({"id": flight_id}, {"$set": update_fields})

    updated = await db.flights.find_one({"id": flight_id}, {"_id": 0})
    for key in ("departure_time", "arrival_time"):
        if isinstance(updated[key], str):
            updated[key] = datetime.fromisoformat(updated[key])
